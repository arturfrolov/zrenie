...
class Validate {
  constructor($form, dispatcher, sender, unlockFieldset = false) {
    this.$form = $form;
  ...
  getMainOptions() {
    ...
    const options = {
      ...
      errorPlacement(error, element) {
        error.appendTo(
          element.closest(`.${cThis.wrapperElement}`).find(`.${cThis.messageElement}`),
        );
      },
      highlight(element) {
  ...
  (function ($) {
    ...
    jQuery.fn.destroyValidateForm = function () {
      function destroyValidate() {
        ...
        $currentForm
          .find(`.${currentValidate.wrapperElement}--error`)

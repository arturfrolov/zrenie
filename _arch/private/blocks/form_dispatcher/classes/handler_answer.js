class HandlerAnswer {
  ...
  parsFormParameters() {
    ...
    this.formParameters.datepicker = {
      id: this.$form.data('id_datepicker'),
      action: 'destroy',
    };
  ...
  followDirective() {
    ...
    if (this.directive.status === 'error') {
      ...
      if (this.directive.error) {
        this.showServerMessage(this.directive.error, this.typeMessage[this.directive.status]);
      }
    }
  }
  ...
  pastTemplate() {
    ...
    if ($target) {
      const promise = $target.empty().prepend($template).promise();
      promise.done(() => {
        if (this.dispatcher) {
          this.dispatcher('handlerAnswer', 'pastTemplate', {
            template: $template,
          });
        }
      });
